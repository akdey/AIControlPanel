...
@register_control(["decision_gate", "ctrl_decision_gate", "decision_gate_node"])
def execute_decision_gate(ctx: PipelineContext, node_config: Dict[str, Any]) -> PipelineContext:
    ...
    Routes payload to 'port_pass' / 'out_pass' or 'port_block' / 'out_block'.
    """
    start_time = time.time()
    block_on_pii = node_config.get("block_on_pii", False)

    # 1. Critical Security Violation (Toxicity, Injection, or Halt signals)
    if "TOXICITY_FLAG" in ctx.taint_flags or "PROMPT_INJECTION" in ctx.taint_flags:
